...
use Zatxm\Validation\File\File;
use Zatxm\Validation\File\UploadedFile;
...
class Validator
{
    ...
    public function each($attribute, $rules)
    {
        ...
        $data = Support::arrDot($this->initializeAttributeOnData($attribute));
        $data = array_merge($data, $this->extractValuesForWildcards($data, $attribute));

        foreach ($data as $key => $value) {
            if (
                Support::strStartsWith($key, $attribute) ||
                (bool)preg_match('/^' . $pattern . '\z/', $key)
            ) {
                foreach ((array)$rules as $ruleKey => $ruleValue) {
                    if (!is_string($ruleKey) || Support::strEndsWith($key, $ruleKey)) {
    ...
    protected function initializeAttributeOnData($attribute)
    {
        ...
        if (!Support::strContains($attribute, '*') || Support::strEndsWith($attribute, '*')) {
            return $data;
        }
        return Support::dataSet($data, $attribute, null, true);
    }
    ...
    public function extractValuesForWildcards($data, $attribute)
    {
        ...
        $data = [];
        foreach ($keys as $key) {
            $data[$key] = Support::arrGet($this->data, $key);
        }
        return $data;
    }
    ...
    protected function mergeRulesForAttribute($attribute, $rules)
    {
        ...
        $merge = Support::arrHead($this->explodeRules([$rules]));
    ...
    {
        if (!is_null($value = Support::arrGet($this->data, $attribute))) {
            return $value;
        } elseif (!is_null($value = Support::arrGet($this->files, $attribute))) {
            return $value;
        }
    }
    ...
    protected function passesOptionalCheck($attribute)
    {
        if ($this->hasRule($attribute, ['Sometimes'])) {
            return array_key_exists($attribute, Support::arrDot($this->data))
    ...
    {
        return Support::arrHas($this->data, $attribute);
    }
    ...
    {
        if (Support::arrHas(array_merge($this->data, $this->files), $attribute)) {
    ...
    protected function validateRequiredIf($attribute, $value, $parameters)
    {
        ...
        $data = Support::arrGet($this->data, $parameters[0]);
    ...
    protected function getPresentCount($attributes)
    {
        $count = 0;
        foreach ($attributes as $key) {
            if (Support::arrGet($this->data, $key) || Support::arrGet($this->files, $key)) {
                $count++;
            }
        }
        return $count;
    ...
    protected function validateInArray($attribute, $value, $parameters)
    {
        ...
        $otherValues = Support::arrWhere(Support::arrDot($attributeData), function ($key) use ($parameters) {
            return Support::strIs($parameters[0], $key);
        });
    ...
    protected function validateSame($attribute, $value, $parameters)
    {
        ...
        $other = Support::arrGet($this->data, $parameters[0]);
    ...
    protected function validateDistinct($attribute, $value, $parameters)
    {
        ...
        $attributeData = $this->extractDataFromPath($explicitPath);
        $data = Support::arrWhere(Support::arrDot($attributeData), function ($key) use ($attribute, $attributeName) {
            return $key != $attribute && Support::strIs($attributeName, $key);
        });
    ...
    {
        $lowerRule = Support::strSnake($rule);
    ...
    protected function getInlineMessage($attribute, $lowerRule, $source = null)
    {
        ...
        foreach ($keys as $key) {
            foreach (array_keys($source) as $sourceKey) {
                if (Support::strIs($sourceKey, $key)) {
    ...
    protected function getCustomMessageFromTranslator($customKey)
    {
        ...
        $customMessages = Support::arrDot((array) $this->translator->trans('validation.custom'));

        foreach ($customMessages as $key => $message) {
            if (Support::strContains($key, ['*']) && Support::strIs($key, $shortKey)) {
                return $message;
            }
        }
        return $customKey;
    ...
    protected function doReplacements($message, $attribute, $rule, $parameters)
    {
        ...
        $snakeRule = Support::strSnake($rule);
        if (isset($this->replacers[$snakeRule])) {
            $message = $this->callReplacer($message, $attribute, $snakeRule, $parameters);
    ...
    protected function getAttribute($attribute)
    {
        ...
        return str_replace('_', ' ', Support::strSnake($attribute));
    }
    ...
    {
        $parameters[1] = $this->getDisplayableValue($parameters[0], Support::arrGet($this->data, $parameters[0]));
    ...
    {
        return Support::arrHas($this->attributes(), $attribute);
    }
    ...
    {
        return [Support::strStudly(trim(Support::arrGet($rules, 0))), array_slice($rules, 1)];
    }
    ...
    protected function parseStringRule($rules)
    {
        ...
        return [Support::strStudly(trim($rules)), $parameters];
    }
    ...
    {
        $results = [];
        $value = Support::arrGet($this->data, $attribute, '__missing__');
        if ($value != '__missing__') {
            Support::arrSet($results, $attribute, $value);
        }
        return $results;
    }
    ...
    {
        if ($extensions) {
            $keys = array_map('\Zatxm\Validation\Support::strSnake', array_keys($extensions));
    ...
    public function addImplicitExtensions(array $extensions)
    {
        ...
        foreach ($extensions as $rule => $extension) {
            $this->implicitRules[] = Support::strStudly($rule);
        }
    }
    ...
    {
        $this->extensions[Support::strSnake($rule)] = $extension;
    }
    ...
    {
        if ($replacers) {
            $keys = array_map('\Zatxm\Validation\Support::strSnake', array_keys($replacers));
    ...
    {
        $this->replacers[Support::strSnake($rule)] = $replacer;
    }
    ...
    {
        if (Support::strContains($callback, '@')) {
    ...
    {
        $rule = Support::strSnake(substr($method, 8));
